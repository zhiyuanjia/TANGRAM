...
BEGIN_MESSAGE_MAP(CWndSliderView, CWnd)
	...
	//ON_MESSAGE(WM_OPENTANGRAMDOCINOBJ,OnOpenDocinTangramObj)
	//ON_MESSAGE(WM_READWRITETANGRAMOBJ,OnSerializeTangramObj)
...
//
//***************************************************************************
int CWndSliderView::CreateView( CString szcViewTitle,
...
void CWndSliderView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if( -1 != nButton )  // The mouse is over a button. 
	{
		...
		if( nButton == m_nMouseOnButton )
		{
			...
			{				
				...
				//if(m_pTangramNode)
				//	m_pTangramNode->m_nActivePage = m_nActiveView;
				RecalcLayout();
				//if(!m_pTangramNode->m_pDoc&&m_pTangramNode->m_pDoc->m_pFrame)
				//	m_pTangramNode->m_pDoc->m_pFrame->RecalcLayout();
	...
	//CRect rc;
	//m_pDoc->m_pFrame->GetClientRect(rc);
	//m_pDoc->m_pFrame->InvalidateRect(&rc);
}
...
//
//***************************************************************************
void CWndSliderView::OnTimer(UINT_PTR nIDEvent)
{
...
//
//***************************************************************************
void CSliderButton::SetButtonTitle( CString szcTitle )
{
...
//
//***************************************************************************
CString CSliderButton::GetButtonTitle( void ) const
{
...
BOOL CWndSliderView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	return CWnd::Create(lpszClassName, lpszWindowName, dwStyle, rect, pParentWnd, nID, pContext);
}

void CWndSliderView::PostNcDestroy()
{
	CWnd::PostNcDestroy();
	DWORD dw = Release();
	//delete this;
...
#endif
BOOL CWndSliderView::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	cs.lpszClass = wndClass.lpszClassName;//AfxRegisterWndClass(0);
...
LRESULT CWndSliderView::OnCreatePage(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CWndSliderView::OnTgmSetCaption(WPARAM wParam, LPARAM lParam)
{
	CSliderButton* pButton = this->GetButton((int)wParam);
	if (pButton)
	{
...
LRESULT CWndSliderView::OnActivePage(WPARAM wParam, LPARAM lParam)
{
	SetActiveView(wParam);
...
LRESULT CWndSliderView::OnModifyPage(WPARAM wParam, LPARAM lParam)
{
	HWND hPageWnd = (HWND)wParam;
	int nViewID = (int)lParam;
	CSliderButton* pButton = GetButton(nViewID);
