...
namespace TangramEclipsePlus
{
	namespace EclipsePlus
	{
		...
		CTangramEventProxy::CTangramEventProxy(JNIEnv *env,
			jobject aSinkObj,
		...
		void CTangramEventProxy::Connect(JNIEnv *env) {
		...
		CTangramEventProxy::~CTangramEventProxy()
		{
			JNIEnv *env;
			Disconnect();
		...
		void CTangramEventProxy::Disconnect() {
			if (connected) {
		...
		STDMETHODIMP CTangramEventProxy::QueryInterface(REFIID rid, void **ppv)
		{
		...
		STDMETHODIMP CTangramEventProxy::GetIDsOfNames(REFIID riid,
		...
		STDMETHODIMP CTangramEventProxy::Invoke(DISPID dispID, REFIID riid,
			...
			VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
		{
			...
			if (DISPATCH_METHOD & wFlags)
			{
				...
				invokeMethod = env->GetMethodID(javaSinkClass, "invoke", "(Ljava/lang/String;[Lcom/tangram/Variant;)Lcom/tangram/Variant;");
				if (env->ExceptionOccurred()) { env->ExceptionDescribe(); env->ExceptionClear(); }
				...
				jmethodID getVariantMethod = env->GetMethodID(javaSinkClass, "getVariant", "()Lcom/tangram/Variant;");
				if (env->ExceptionOccurred()) { env->ExceptionDescribe(); env->ExceptionClear(); }
				...
				//Java_com_tangram_Variant_release(env, aVariantObj);
				...
				for (i = numVariantParams - 1, j = 0; i >= 0; i--, j++)
				{
					...
					//Java_com_tangram_Variant_release(env, arg);
					zeroVariant(env, arg);
					env->DeleteLocalRef(arg);
				}
		...
		void CTangramEventProxy::convertJavaVariant(VARIANT *java, VARIANT *com) {
