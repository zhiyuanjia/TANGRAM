...
HRESULT CTangramEditorDoc<Traits_T>::ReadData(
	File& rFile, 
	BOOL bInsert,
	DWORD& rdwFormatIndex) throw()
{
	...
	if (theApp.m_pTangramCore)
	{
		...
		{
			m_bEditModel = m_Parse.attrBool(_T("EditModel"), FALSE);
			if (m_bEditModel)
			{
				CTangramXmlParse* pParse = m_Parse.GetChild(_T("window"));
				if (pParse)
				{
					//m_strKey = pParse->attr(_T("toolwindowkey"), _T(""));
					//if (m_strKey != _T(""))
					//{
					//	CComVariant bstrVal(L"");
					//	theApp.m_pTangramCore->get_TangramVal(CComBSTR(m_strKey), &bstrVal);
					//	CString strVal = OLE2T(bstrVal.bstrVal);
					//	if (strVal == _T(""))
					//	{
					...
					//			{
					//				theApp.m_pTangramCore->put_TangramVal(CComBSTR(m_strKey), CComVariant(m_Parse.xml()));
					//			}
					//		}
					//	}
					//}

					HWND h = ::GetParent(GetControl().m_hWnd);
					theApp.m_pTangramCore->CreateTangram((LONGLONG)::GetParent(h), &m_pTangram);
					if (m_pTangram)
					{
						//m_pTangram->put_External((IDispatch*)theApp.m_pApplication);
						//CComBSTR strAppStartFilePath(L"c:\\SplitterTab.xtml");
						//_AtlModule.m_pTangramCore->get_TangramVal(CComBSTR(L"AppPath"), &strAppStartFilePath);
						...
						if (pITextDocument)
						{
							m_pNode->put_Extender(pITextDocument);
						}
			...
			else
			{
				...
				if (strUrl != _T(""))
				{
					...
					if (m_pTangram)
					{
						m_pTangram->put_URL(strUrl.AllocSysString());
					}
				}
				else
				{
					...
					if (pParse)
					{
						...
						if (m_pTangram)
						{
							...
							m_pFrame->Extend(CComBSTR(L""), bstr, &m_pNode);
							m_pNode->put_VSToolWndScript(CComBSTR(m_strKey));
							::PostMessage(theApp.m_hVSToolWnd, WM_ACTIVEAPPXMLDOC, (WPARAM)m_pNode, 0);
						}
					}
...
template <class Traits_T>
void CTangramEditorDoc<Traits_T>::WriteData(File& rFile, DWORD /*dwFormatIndex*/)
{
...
DWORD CALLBACK CTangramEditorDoc<Traits_T>::EditStreamCallback(
...
template <class Traits_T>
bool CTangramEditorDoc<Traits_T>::IsValidFormat(DWORD dwFormatIndex)
{
...
template <class Traits_T>
void CTangramEditorDoc<Traits_T>::OnFileChangedSetTimer()
{
	// 500 milliseconds is an arbitrary time to delay
	// See CTangramEditorDoc::OnTimer and 
...
template <class Traits_T>
const GUID& CTangramEditorDoc<Traits_T>::GetEditorTypeGuid() const
{
...
template <class Traits_T>
void CTangramEditorDoc<Traits_T>::GetFormatListString(ATL::CStringW& rstrFormatList)
{
...
template <class Traits_T>
void CTangramEditorDoc<Traits_T>::PostSetDirty()
{
...
template <class Traits_T>
void CTangramEditorDoc<Traits_T>::PostSetReadOnly()
{
